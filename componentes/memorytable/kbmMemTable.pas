...
//
// Copyright 1999-2003 Kim Bo Madsen/Components4Developers
// All rights reserved.
//
// LICENSE AGREEMENT
...
{$ENDIF}
     ...
     function FindRecordNumber(const RecordBuffer:PChar):integer;
  ...
  public
     ...
     procedure ReflectToIndexes(const How:TkbmIndexUpdateHow; const OldRecord,NewRecord:PkbmRecord; const RecordPos:integer);
...
{$ENDIF}
  ...
  public
      ...
      procedure ReflectToIndexes(const Caller:TkbmCustomMemTable; const How:TkbmIndexUpdateHow; const OldRecord,NewRecord:PkbmRecord; const RecordPos:integer);
...
{$ENDIF}
        ...
        function CopyRecords(Source,Destination:TDataSet; Count:longint):longint; {$IFDEF BCB}dynamic{$ELSE}virtual{$ENDIF};
        ...
        function AddFilteredIndex(const Name, Fields: string; Options: TIndexOptions; Filter:string; FilterOptions:TFilterOptions; FilterFunc:TkbmOnFilterIndex {$ifdef LEVEL6} = nil{$endif}):TkbmIndex;
...
{$ENDIF}
        ...
        procedure SaveToDataSet(Destination:TDataSet); {$IFDEF BCB}dynamic{$ELSE}virtual{$ENDIF};
...
{$IFNDEF LEVEL3}
                                        ,ftFixedChar,ftLargeInt,ftADT,ftArray
{$ENDIF}
{$IFDEF LEVEL5}
...
const
  ...
  FldTypeMap: TFieldMap = (
    ...
    ord(ftArray), ord(ftUnknown), ord(ftUnknown), ord(ftBlob), ord(ftBlob), ord(ftUnknown), ord(ftUnknown),
    ord(ftUnknown), ord(ftGUID) {$ifdef LEVEL6},ord(ftTimeStamp), ord(ftFmtBCD){$endif} );
{$ENDIF}
     ...
     begin
          ...
          begin
               ...
               begin
                    ...
                    else if (pFieldSrc[0]=kbmffNull) and (pVarLengthDest^<>nil) then
                    begin
                         FreeVarLength(pVarLengthDest^);
                         pVarLengthDest^:=nil;
                         pFieldDest[0]:=kbmffNull;
                    end;
               end;
          end;
     end;
...
begin
     ...
     begin
          ...
          pVarLenDest^:=nil;
          pFldDest[0]:=kbmffNull;  // Set field value to NULL.
     end;
     ...
     begin
          ...
          pFldDest[0]:=kbmffData;  // Set field value to NOT NULL.
     end;
end;
...
{$ENDIF}
     ...
     begin
          ...
          else if (p1[0]<>kbmffNull) then Result:=1
          else if (p2[0]<>kbmffNull) then Result:=-1
          else Result:=0;
...
procedure TkbmCommon.LayoutRecord;
...
begin
     ...
     FFixedRecordSize:=0;
     FFieldCount:=0;
     EnumerateFieldDefs(FOwner.FieldDefs,FFieldCount);
...
procedure TkbmCommon.ReflectToIndexes(const Caller:TkbmCustomMemTable; const How:TkbmIndexUpdateHow; const OldRecord,NewRecord:PkbmRecord; const RecordPos:integer);
var
   i:integer;
   mt:TkbmCustomMemTable;
   rp:integer;
begin
     ...
     try
        ...
        begin
             ...
             if mt.Active then
                mt.Indexes.ReflectToIndexes(How,OldRecord,NewRecord,rp);
        end;
     finally
        Unlock;
     end;
     ...
     try
        ...
        begin
             for i:=0 to FRecords.Count-1 do
                 ...
                 begin
                      if not self.Filter(PkbmRecord(FRecords.Items[i])) then continue;
                      FReferences.Add(FRecords.Items[i]);
                 end;
        end;
     finally
        FDataSet.FCommon.Unlock;
...
begin
     ...
     lIndex:=TkbmIndex.Create(IndexDef.Name,FDataSet,IndexDef.Fields,IndexOptions2CompareOptions(IndexDef.Options),mtitSorted,false);
     AddIndex(lIndex);
end;
...
procedure TkbmIndexes.ReflectToIndexes(const How:TkbmIndexUpdateHow; const OldRecord,NewRecord:PkbmRecord; const RecordPos:integer);
var
   i,ni:integer;
   lIndex:TkbmIndex;
               ...
               case How of
                    mtiuhEdit:
                       begin
                            ...
                            if lIndex.CompareRecords(lIndex.FIndexFieldList,OldRecord,NewRecord,false,false)=0 then
                            begin
...
{$IFNDEF LEVEL3}
     if Field.DataType in [ftWideString{$IFDEF LEVEL5},ftGUID{$ENDIF}] then
        cl:=TStringField
     else
     begin
...
begin
     ...
     try
        ...
        if FCommon.FOwner=self then FCommon.LayoutRecord;
...
begin
     Result:=(FCommon._InternalCompareRecords(FIndexList,-1,FKeyBuffers[kbmkbRangeStart],ARecord,true,false,chBreakGT)<=0)
             and (FCommon._InternalCompareRecords(FIndexList,-1,FKeyBuffers[kbmkbRangeEnd],ARecord,true,false,chBreakLT)>=0);
end;
...
begin
     ...
     FCommon.ReflectToIndexes(self,mtiuhInsert,nil,pCopyRec,where);
...
begin
     ...
     try
        ...
        // Update indexes.
        FCommon.ReflectToIndexes(self,mtiuhDelete,pRec,nil,FRecNo);
...
{$ENDIF}
     ...
     try
        ...
        begin
             ...
             begin
                  ...
                  begin
                       ...
                       if (FCommon.FVersioningMode=mtvmAllSinceCheckPoint) or
                          ((FCommon.FVersioningMode=mtvm1SinceCheckPoint) and (pActRec^.UpdateStatus=usUnModified)) then
                       begin
             ...
             // Update index.
             FCommon.ReflectToIndexes(self,mtiuhEdit,pRec,pActRec,FRecNo);
        ...
        begin
             ...
             if GetBookmarkFlag(PChar(pNewRec))=bfEOF then
                FCommon.ReflectToIndexes(self,mtiuhInsert,nil,pNewRec,-1)
             else
                FCommon.ReflectToIndexes(self,mtiuhInsert,nil,pNewRec,FInsertRecNo);
...
begin
     ...
     if ARecord^.UpdateStatus=usDeleted then
     begin
          FCommon.ReflectToIndexes(self,mtiuhDelete,ARecord,nil,RecordIndex);
          ...
          case Destination.Fields[i].FieldKind of
               ...
               fkData,fkInternalCalc,fkCalculated:
                 begin
                      ...
                      if not (Destination.Fields[i].DataType in (kbmSupportedFieldTypes{$IFDEF LEVEL4}-[ftWideString]{$ENDIF})) then
                      begin
                           fi[i]:=-1;
                           continue;
                      end;
          ...
          case Destination.Fields[i].FieldKind of
               ...
               fkData,fkInternalCalc,fkCalculated:
                 begin
                      ...
                      if not (Destination.Fields[i].DataType in (kbmSupportedFieldTypes-[ftAutoInc]{$IFDEF LEVEL4}+[ftWideString]{$ENDIF})) then
                      begin
                           fi[i]:=-1;
                           continue;
                      end;
...
begin
     ...
     try
        ...
        try
           ...
           // Move to first record in this.
           First;
           FSaveCount:=CopyRecords(self,Destination,FSaveLimit);
        finally
           Destination.First;
        end;
