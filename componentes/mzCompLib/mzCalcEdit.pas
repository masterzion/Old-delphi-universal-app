...
uses
  SysUtils, Classes, QControls, QStdCtrls, QExtCtrls, QGraphics,
  QComCtrls, Math, QForms, QGrids, Types, QButtons, Qt,
  mzCustomBtnEditclx, mzCompLib;


type
  TEndCalc = procedure(Calc : string) of object;
  TOperator = (opNull, opMais, opMenos, opDividir, opVezes);
  TmzCalc = class(TFrameControl)
  private
  ...
  published
    ...
    property OnEndCalc :  TEndCalc read FEndCalc  write FEndCalc;
  end;



 TDropDownCalc = class(TPersistent)
  private
    { Private declarations }
    FWidth: Integer;
    FHeight: Integer;
  ...
  TmzCalcEdit = class(TmzCustomBtnEditclx)
  private
    { Private declarations }
    FOldValue : string;
    FDropDownCalc : TDropDownCalc;
    bCriado : Boolean;
    function ReadDropDownCalc: TDropDownCalc;
    procedure SetDropDownCalc(const Value: TDropDownCalc);
  protected
    { Protected declarations }
  ...
  published
    ...
    property OnResize;
    property OnExit;
    property DropDownCacl: TDropDownCalc read ReadDropDownCalc write SetDropDownCalc;
    property ShowHint;
...
// ************************ TmzCalc ************************
constructor TmzCalc.Create(AOwner: TComponent);
begin
...
procedure TmzCalc.Clear;
begin
   btnCE.Click;
end;

procedure TmzCalc.btnNumClick(Sender: TObject);
var
  S: String;
  Value: Extended;
begin
...
procedure TmzCalc.btnLimpaClick(Sender: TObject);
begin
  Edit.Text := '0';
  bDecimal := False;
  Operador := opNull;
...
procedure TmzCalc.btnIgualClick(Sender: TObject);
var
  Op2, Result: Extended;
  sOpera : string;
begin
...
procedure TmzCalc.btnPercClick(Sender: TObject);
var
...
procedure TmzCalc.btnOperaClick(Sender: TObject);
begin
  btnIgual.Click;
...
procedure TmzCalc.btnPontoClick(Sender: TObject);
begin
  bDecimal := True;
end;

procedure TmzCalc.EditKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  case Key of
...
function TmzCalc.GetValue: string;
begin
  Result := Edit.Text;
end;


procedure TmzCalc.SetValue(Valor:String);
begin
  try
...
procedure TmzCalc.btnCClick(Sender: TObject);
var
  S: String;
begin
...
// ************************ TDropDownCalc ************************
constructor TDropDownCalc.Create;
begin
  FWidth := 0;
  FHeight := 0;
end;

procedure TDropDownCalc.SetWidth(const Value: Integer);
begin
  FWidth := Value;
end;

procedure TDropDownCalc.SetHeight(const Value: Integer);
begin
  FHeight := Value;
end;
...
// ************************ TDropDownCalc ************************
constructor TmzCalcEdit.Create(AOwner: TComponent);
begin
  FOldValue := '0';
  inherited Create(AOwner);
  FDropDownCalc := TDropDownCalc.Create;
  btnEdit.Glyph.LoadFromResourceName(HInstance,'TMZCALCMINI');
...
procedure TmzCalcEdit.ExecuteClick;
var
  MyTempForm: TForm;
  MyTempCalc: TmzCalc;
  MyPoint: TPoint;
begin
  ...
  try
   with MyTempForm do begin
     ...
     MyTempCalc := TmzCalc.Create(MyTempForm);
...
destructor TmzCalcEdit.Destroy;
begin
...
function TmzCalcEdit.ReadDropDownCalc: TDropDownCalc;
begin
  Result := FDropDownCalc;
end;


procedure TmzCalcEdit.SetDropDownCalc(const Value: TDropDownCalc);
begin
  FDropDownCalc.Assign(Value);
end;

procedure TmzCalcEdit.Change;
begin
  inherited;
...
procedure Register;
begin
  RegisterComponents('Master', [TmzCalc]);
  RegisterComponents('Master', [TmzCalcEdit]);
end;
